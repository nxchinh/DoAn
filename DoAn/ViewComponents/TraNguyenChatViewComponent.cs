...
{
    public class TraNguyenChatViewComponent : ViewComponent
    {
        ...
        public TraNguyenChatViewComponent(IMemoryCache cache)
        {
            _cache = cache;
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            string cacheKey = "TraNguyenChat";
            if (!_cache.TryGetValue(cacheKey, out cacheEntry))
            {
                cacheEntry = await _db.SanPhams.Where(x => x.MaLoaiSanPham == 4).ToListAsync();
