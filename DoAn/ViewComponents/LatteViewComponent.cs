...
{
    public class LatteViewComponent:ViewComponent
    {
        ...
        public LatteViewComponent(IMemoryCache cache)
        {
            _cache = cache;
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            string cacheKey = "Latte";
            if (!_cache.TryGetValue(cacheKey, out cacheEntry))
            {
                cacheEntry = await _db.SanPhams.Where(x => x.MaLoaiSanPham == 6).ToListAsync();
