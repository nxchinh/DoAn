...
{
    public class DacBietEstoreViewComponent : ViewComponent
    {
        ...
        public DacBietEstoreViewComponent(IMemoryCache cache)
        {
            _cache = cache;
        }
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            string cacheKey = "DacBietEstore";
            if (!_cache.TryGetValue(cacheKey, out cacheEntry))
            {
                cacheEntry = await _db.SanPhams.Where(x => x.MaLoaiSanPham == 8).ToListAsync();
